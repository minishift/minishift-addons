...
NOTE: The add-on was tested with Istio release v0.2.12
...
The following PowerShell commands show how to download Istio 0.2.12 version for Windows and set the `ISTIO_HOME` environment variable:

[code,powershell]
----
# $url = "https://github.com/istio/istio/releases/download/0.2.12/istio_0.2.12_win.zip"
# $Path = Get-Location
# $output = "$Path\istio_0.2.12_win.zip”
# (New-Object System.Net.WebClient).DownloadFile($url, $output)
# Expand-Archive $output -DestinationPath $Path
# [Environment]::SetEnvironmentVariable("ISTIO_HOME", "$Path\istio-0.2.12", "User")
----
